...
public class GETClient {
	...
	public static void Hello(String args[]) throws Exception {
		...
		if (args.length >= 0) {
			...
			try {
				uri = new URI("coap://localhost:3005/helloWorld");
			...
			if (response!=null) {
				...
				if (args.length > 1) {
					try (FileOutputStream out = new FileOutputStream(new File("C:\\Users\\Sumit\\workspace\\UserManagement\\src\\com\\tutorialspoint\\Hello"))) {
					...
					PrintWriter out1 = new PrintWriter(new FileOutputStream(
						    new File("C:\\Users\\Sumit\\Desktop\\Hello"), 
						    true /* append = true */)); 
					//PrintWriter out1 = new PrintWriter(new FileOutputStream( new File("C:\\Users\\Sumit\\Desktop\\Hello") ),true);
					out1.println(response.getResponseText());
					out1.flush();
					out1.close();
